import p1_img from './oil_seal_and_rubber_parts1.png'
import p2_img from './oil seal and rubber parts2.png'
import p3_img from './oil seal and rubber parts3.png'
import p4_img from './oil seal and rubber parts4.png'
import p5_img from './gear_part_1.png'
import p6_img from './rubber_ring1.webp'
import p7_img from './gear_part_2.png'
// import p8_img from './product_8.png'
// import p9_img from './product_9.png'
// import p10_img from './product_10.png'
// import p11_img from './product_11.png'
// import p12_img from './product_12.png'

const all_products = [
  {
    id: 1,
    name: "Premium Oil Seals & Rubber Rings",
    desc: "High-quality oil seals and rubber rings for all tractor models.",
    category: 'Oil Seals',
    badge: 'Bestseller',
    image: p1_img,
    new_price: 50.00,
    old_price: 80.50,
  },
  {
    id: 2,
    name: "Premium Oil Seals & Rubber Rings",
    desc: "Industrial grade rubber O-rings for hydraulic and fuel systems.",
    category: 'Oil Seals',
    badge: 'Bestseller',
    image: p2_img,
    new_price: 85.00,
    old_price: 120.50,
  },
  {
    id: 3,
    name: "Premium Oil Seals & Rubber Rings",
    desc: "Durable crankshaft oil seal for high-temperature environments.",
    category: 'Oil Seals',
    badge: 'Bestseller',
    image: p3_img,
    new_price: 60.00,
    old_price: 100.50,
  },
  {
    id: 4,
    name: "Premium Oil Seals & Rubber Rings",
    desc: "Multi-purpose rubber gaskets for engine and exhaust sealing.",
    category: 'Oil Seals',
    badge: 'Bestseller',
    image: p4_img,
    new_price: 100.00,
    old_price: 150.00,
  },
  {
   id: 5,
    name: "Premium Gear Parts",
    desc: "Heavy-duty precision gear parts for smooth and reliable tractor performance.",
    category: 'Gear Parts',
    badge: 'Bestseller',
    image: p5_img,
    new_price: 45.00,
    old_price: 70.00,
  },
  {
    id: 6,
    name: 'Rubber Gasket Set',
    desc: 'Multi-purpose rubber gaskets for engine and exhaust sealing.',
    category: 'Rubber Parts',
    badge: "popular",
    new_price: 30,
    old_price: 50,
    image: p6_img,
  },
  {
    id: 7,
    name: 'Differential Gear Pair',
    desc: 'Precision-cut differential gears for tractor rear axle.',
    category: 'Gears',
    badge: 'Popular',
    new_price: 110,
    old_price: 160,
    image: p7_img,
  },
  // {
  //   id: 8,
  //   name: 'Nut & Washer Combo Pack',
  //   desc: 'Assorted nuts and washers for general tractor maintenance.',
  //   category: 'Nuts & Bolts',
  //   badge: null,
  //   new_price: 25,
  //   old_price: 40,
  //   image: p8_img,
  // },
  // {
  //   id: 9,
  //   name: 'Front Axle Oil Seal',
  //   desc: 'Leak-proof front axle seal for long-lasting performance.',
  //   category: 'Oil Seals',
  //   badge: 'New',
  //   new_price: 55,
  //   old_price: 85,
  //   image: p9_img,
  // },
  // {
  //   id: 10,
  //   name: 'Rubber Dust Boot Kit',
  //   desc: 'Protective rubber boots for steering and suspension joints.',
  //   category: 'Rubber Parts',
  //   badge: null,
  //   new_price: 40,
  //   old_price: 65,
  //   image: p10_img,
  // },
  // {
  //   id: 11,
  //   name: 'PTO Gear Assembly',
  //   desc: 'Power take-off gear assembly for tractor implement drive.',
  //   category: 'Gears',
  //   badge: 'Bestseller',
  //   new_price: 130,
  //   old_price: 190,
  //   image: p11_img,
  // },
  // {
  //   id: 12,
  //   name: 'Foundation Bolt Set',
  //   desc: 'Heavy-duty foundation bolts for engine mounting and frame.',
  //   category: 'Nuts & Bolts',
  //   badge: 'Popular',
  //   new_price: 75,
  //   old_price: 110,
  //   image: p12_img,
  // },
]

export default all_products